...
export function Header() {
  ...
  return (
    ...
    >
      {/* ======================================================== */}
      {/* TOP BAR: BADGE DE CONFIANZA SEP (RESPONSIVE)               */}
      {/* ======================================================== */}
      <div className="w-full bg-slate-900 text-white/90 py-1.5 px-2 flex items-center justify-center gap-1.5 text-[10px] sm:text-xs font-semibold tracking-widest uppercase border-b border-white/10 z-50">
        <ShieldCheck className="h-3.5 w-3.5 text-brand-primary shrink-0" />
        <span className="text-center line-clamp-1">
          <span className="sm:hidden">Estudios con Validez Oficial SEP (RVOE)</span>
          <span className="hidden sm:inline">Institución con Reconocimiento de Validez Oficial de Estudios (RVOE) por la SEP</span>
        </span>
      </div>
      ...
      <div className="container mx-auto flex h-16 sm:h-20 items-center justify-between px-4 md:px-6">
        ...
        <Link href="/" className="flex items-center gap-2 sm:gap-3 group">
          <div className="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-primary to-brand-highlight shadow-lg shadow-brand-primary/20 group-hover:scale-105 transition-transform duration-300">
            <GraduationCap className="h-6 w-6 sm:h-7 sm:w-7 text-white" />
          </div>
          <div className="flex flex-col">
            <span className={`text-sm sm:text-base font-extrabold leading-tight tracking-tight transition-colors ${scrolled ? 'text-foreground' : 'text-white'}`}>
              Instituto Universitario
            </span>
            <span className="text-[10px] sm:text-xs font-medium text-brand-primary uppercase tracking-wider">
              de Durango
            </span>
          </div>
        </Link>
        ...
        <div className="hidden items-center gap-4 md:flex">
          <Button variant="ghost" size="sm" className={`font-semibold hover:text-brand-primary hover:bg-brand-primary/10 rounded-full px-5 ${scrolled ? 'text-muted-foreground' : 'text-white'}`} asChild>
          ...
          <Button size="sm" className="rounded-full bg-brand-primary hover:bg-brand-primary/90 text-white shadow-md shadow-brand-primary/20 px-6 transition-all hover:scale-105" asChild>
            <Link href="/inscripcion">Inscripciones</Link>
          </Button>
        </div>
        ...
        <nav className="container flex flex-col gap-2 px-4 py-6">
          ...
          <div className="flex flex-col gap-3 pt-6 mt-2 border-t border-brand-primary/10">
            <Button variant="outline" className="w-full rounded-full border-brand-primary/20 text-brand-primary h-12" asChild>
              <Link href="/login" onClick={() => setMobileMenuOpen(false)}>Portal Alumnos</Link>
            </Button>
            <Button className="w-full rounded-full bg-brand-primary text-white h-12 shadow-md shadow-brand-primary/20" asChild>
              <Link href="/inscripcion" onClick={() => setMobileMenuOpen(false)}>Proceso de Admisión</Link>
            </Button>
          </div>
        </nav>
      </div>

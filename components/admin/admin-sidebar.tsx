        ...
        <Link href="/admin" className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="flex flex-col">
              <span className="text-sm font-black uppercase">Admin IUD</span>
              <span className="text-[10px] font-bold text-brand-primary uppercase tracking-widest">Panel</span>
            </div>
          )}
        </Link>
      </div>
        ...
        <div className={cn('flex items-center gap-3 mb-3', collapsed && 'justify-center')}>
          <Avatar className="h-9 w-9">
            <AvatarFallback className="bg-brand-primary text-white text-xs font-bold">
              {perfil ? getInitials(getNombrePerfil(perfil)) : 'A'}
            </AvatarFallback>
          </Avatar>
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="truncate text-sm font-bold">{getNombrePerfil(perfil)}</p>
              <p className="text-xs text-brand-primary">Administrador</p>
            </div>
          )}
        </div>

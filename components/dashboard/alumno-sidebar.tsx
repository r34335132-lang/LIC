        ...
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-brand-primary uppercase tracking-widest">Campus</span>
            </div>
          )}
        </Link>
      </div>
        ...
        <div className={cn('flex items-center gap-3 mb-3', collapsed && 'justify-center')}>
          <Avatar className="h-9 w-9">
            <AvatarFallback className="bg-brand-primary text-white text-xs font-bold">
              {perfil ? getInitials(getNombrePerfil(perfil)) : 'E'}
            </AvatarFallback>
          </Avatar>
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="truncate text-sm font-bold">{getNombrePerfil(perfil)}</p>
              <p className="text-xs text-muted-foreground">{perfil?.matricula}</p>
            </div>
          )}
        </div>

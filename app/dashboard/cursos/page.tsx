...
import { cursos, getProfesorByCurso } from "@/lib/data" // CORRECCIÓN: Importamos 'cursos' y no 'mockCourses'
...
// Mantenemos tu contenido de prueba
const mockContent = [
...
export default function ContenidoPage() {
  ...
  // CORRECCIÓN: Tu base de datos usa "rol" en lugar de "role"
  const canUpload = user?.rol === "maestro" || user?.rol === "admin"

  return (
    <div className="space-y-8 animate-fade-in pb-10">
      
      {/* Header Premium (Igual que en Mis Cursos) */}
      ...
      {/* Tarjetas de Cursos (Corregidas para usar la base de datos real) */}
      <div className="pt-4">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {/* CORRECCIÓN: Ahora iteramos sobre 'cursos' que importamos de data.ts */}
              ...
              <Card key={curso.id} className="group cursor-pointer border-border/40 bg-white/60 dark:bg-black/40 backdrop-blur-md hover:-translate-y-1 hover:shadow-xl hover:border-brand-primary/30 transition-all overflow-hidden rounded-2xl">
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="font-bold text-lg leading-tight group-hover:text-brand-primary transition-colors">{curso.nombre}</h3>
                      {/* CORRECCIÓN: mapeamos profe?.nombre en lugar de course.teacher */}
                      <p className="text-sm text-muted-foreground mt-1">{profe?.nombre || 'Sin profesor'}</p>
                    </div>
                  </div>
